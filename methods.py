...
def show_duplicates(df): 
    ...
    st.write("Check for duplicate rows below:")
    duplicates = df[df.duplicated(keep=False)]  # Keep all duplicates
    if duplicates.empty: 
        st.write("No duplicate rows found.") 
    else: 
        st.write("Num Of duplicated Rows Is :",df.duplicated().sum() )
...
#             # Update the session state to store the modified DataFrame
#             st.session_state['data'] = df
#             st.session_state['duplicates_handled'] = True
            
#             # Display a success message and the updated DataFrame
...
def handle_duplicates(df):
    ...
    if num_duplicates > 0:
        ...
        if st.button("Remove Duplicate Rows", key='remove_duplicates'):
            ...
            st.session_state['duplicates_handled'] = True  # Set flag
            st.success("Duplicate rows removed successfully!")
    else:
        st.info("No duplicate rows to remove.")
...
def column_names_analysis(df):
    ...
    if rename_button:
        try:    
            ...
            #reset_all_flags()  # Edited :  removed
...
def reset_all_flags():
    ...
    keys_to_reset = [
        ...
        'visualize_data_run', 'correlation_run','type_converted',
        'columns_renamed','data_type_analysis_clicked',
        'column_name_analysis_clicked','data_info'
    ]
